		...
		tsf = ts.filter = {
				...
				notMatch: function( c, data ) {
					if ( tsfRegex.notTest.test( data.iFilter ) ) {
						...
						if ( tsfRegex.exact.test( filter ) ) {
							...
							return filter === '' ? true : ((filter == null ? '' : String(filter)).trim()) !== data.iExact;
						} else {
							indx = data.iExact.search((filter == null ? '' : String(filter)).trim());
							return filter === '' ? true :
					...
					if ( $header && $header.length ) {
						...
						if ( buildFilter ) {
							...
							$.each(tmp, function(indx, attr) {
								...
								name = name.replace( regex, (text == null ? '' : String(text)).trim() );
							});
							buildFilter.attr({
			...
			defaultFilter: function( filter, mask ) {
				...
				var regex = tsfRegex.iQuery,
					...
					query = maskLen > 1 ? ((filter == null ? '' : String(filter)).trim()).split(/\s/) : [ (filter == null ? '' : String(filter)).trim() ],
					len = query.length - 1,
					indx = 0,
					val = mask;
				if ( len < 1 && maskLen > 1 ) {
