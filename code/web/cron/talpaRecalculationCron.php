...
if (!array_key_exists('Talpa Search', $enabledModules)) {
	$logger->log("Talpa module not enabled, quitting", Logger::LOG_DEBUG);
	return;
}
...
if (!$talpaSettings->find(true)) {
	$logger->log("No Talpa settings found, exiting", Logger::LOG_DEBUG);
	exit(1);
}
...
if ($results) {
	while ($result = $results->fetch()) {
		$logger->log('found '. $result['total'] . ' permanent IDs to send to Talpa for recalculation', Logger::LOG_DEBUG);
	}
}
	...
	while ($result = $results->fetch()) {
		...
		if( count($permanent_ids) > $BATCH_SIZE ) {
			...
			foreach ($chunks as $chunk) {
				...
				$logger->log($resA['msg'], Logger::LOG_WARNING, true);

				//item has been received for processing. Mark as checked.
				if($resA['status']=='ok') {
					foreach ($chunk as $permanent_id => $data) {
						$logger->log('permanent id: '.$permanent_id, LOG_WARNING, true);
					...
					$logger->log("Something went wrong with sending your records to Talpa. Please try again later.", Logger::LOG_DEBUG);
				}
			} // foreach chunksA
