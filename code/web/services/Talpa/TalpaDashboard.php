...
class Talpa_TalpaDashboard extends Admin_Dashboard {
	function launch() {
		global $interface;

		$instanceName = $this->loadInstanceInformation('TalpaRecordUsage');
		$this->loadDates();
		...
		$this->display('talpaDashboard.tpl', 'Talpa Dashboard');
	}
	...
	public function getRecordStats($instanceName, $month, $year): array {
		...
		$usage->selectAdd('COUNT(talpaId) as recordsUsed');
